...
class MLP {
 ...
 public:
  ...
  void entrenar(matriz& entrada, matriz& salida, int n_iteracion,
  ...
  double testing(matriz& entrada, matriz& salida) {
    ...
    int correctos = 0;
    for (int i = 0; i < entrada.size(); i++) {
      double min_val = *min_element(entrada[i].begin(), entrada[i].end());
      double max_val = *max_element(entrada[i].begin(), entrada[i].end());

      for (double& value : entrada[i]) {
